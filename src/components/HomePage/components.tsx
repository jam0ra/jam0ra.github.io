  ...
  <article className="bg-[#2A2A2A] rounded-xl p-6 hover:ring-1 hover:ring-[#1E90FF] transition-all">
    <h3 className="text-xl font-semibold mb-2 flex items-center">
      ...
      {githubUrl && (
        <Link 
          href={githubUrl} 
          className="ml-2 text-[#1E90FF] hover:text-blue-400 focus:outline-none focus:ring-2 focus:ring-[#1E90FF]"
    ...
    <div className="flex flex-wrap gap-2" role="list" aria-label="Technologies used">
      {technologies.map((tech) => (
        <span 
          key={tech} 
          className="px-3 py-1 bg-[#1E90FF]/10 text-[#1E90FF] rounded-full text-sm"
          role="listitem"
        >
          {tech}
        </span>
...
const ExperienceCard: React.FC<ExperienceCardProps> = ({ title, company, date, responsibilities }) => (
  <article className="bg-[#2A2A2A] rounded-xl p-6 hover:ring-1 hover:ring-[#1E90FF] transition-all">
    ...
    <ul className="text-gray-300 space-y-2" aria-label="Job responsibilities">
      {responsibilities.map((responsibility, index) => (
        <li key={index} className="flex items-start">
          <span className="text-[#1E90FF] mr-2" aria-hidden="true">•</span>
          {responsibility}
        </li>
      ))}
    </ul>
...
const DockItem: React.FC<DockItemProps> = ({ icon, label, active, onClick }) => (
  ...
  >
    ...
    <span 
      className="absolute left-16 px-2 py-1 bg-[#2A2A2A] text-sm rounded-md opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap"
      role="tooltip"
    >
      {label}
    </span>
